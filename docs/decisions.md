...
## ADR-020 — Abstracción de proveedores + fallback cloud local-first (modo HYBRID)

- **Fecha:** 2026-09-09
- **Contexto:** PHASE 14 quiere un segundo backend de IA (OpenCode / proveedores cloud) para tareas pesadas que el modelo local no cubre con recursos suficientes, sin romper el modo LOCAL existente ni el compromiso local-first (ADR-013). El stack ya tiene la abstracción `LLMProvider` (ADR-003/ADR-006) y el Model Router decide el modelo (ADR-014); la tentación sería un sistema paralelo de OpenAI-compatible con API keys propias y listas de modelos gratuitos, que duplicaría la arquitectura y filtraría credenciales.
- **Decisión:**
  1. **Reutilizar la abstracción `LLMProvider`**: `OpenCodeProvider` (`nova/llm/opencode.py`) implementa exactamente la misma interfaz (`chat`/`list_models`/`health`/`close`) y se registra en el mismo `ProviderRegistry` como `opencode`. El Router habla con proveedores, nunca con comandos CLI. No hay sistema paralelo.
  2. **OpenCode vía HTTP, sin SDK ni API keys**: N.O.V.A. conecta al servidor propio de OpenCode (`http://127.0.0.1:4096`) con httpx, igual que habla con Ollama. El auth lo gestiona el propio OpenCode (`~/.local/share/opencode/auth.json` o env); N.O.V.A. **nunca** pide credenciales directamente. Una sesión efímera por `chat()` evita estado cruzado.
  3. **Diagnóstico por capas, no por claves**: `detect_opencode()` reporta instalado/versión/servidor/proveedores/configurado/models sin escribir nada. No se hardcodea una lista de modelos "gratuitos"; si el coste no es fiable, se reporta `unknown`.
  4. **Config nueva sin duplicar**: `ai.mode` (`local`|`hybrid`) y `ai.privacy` (`local_only`|`cloud_allowed`), más `open_code.*` y `model_router.strategy: local-first`. Defaults: `local` + `local_only` = el modo LOCAL actual, byte a byte. El `NovaSettings` vuelve a parsear el config existente sin romperlo.
  5. **El router decide proveedor + modelo**: `RoutingDecision` gana `provider`, se decide con `_cloud_for_task` y caso por caso. Local-first: **solo** una tarea `heavy` con recursos locales insuficientes puede ir a cloud, y solo si hybri + privacy lo permiten y hay modelo cloud mapeado. `simple`/`general` nunca van a cloud. `/v1/route` sigue compatible (añade `provider`).
  6. **Fallback bidireccional**: cloud->local en CLI/API si el proveedor cloud falla; local->cloud según el router. Desactivado por completo si `local_only`.
  7. **Privacidad y memoria**: la memoria queda local (SQLite + embeddings Ollama); no se mandan bases de memoria a cloud. Cloud solo ve el prompt del turno y solo cuando la política lo permite.
  8. **Ownership en setup/update/remove**: N.O.V.A. **nunca instala OpenCode** ni toca su config/auth externa. `setup` pregunta Local/Hybrid/Configure later; `update` re-verifica OpenCode y ofrece refrescar el catálogo cloud con confirmación; `remove` elimina solo las secciones `ai`/`open_code` del config propiedad de N.O.V.A.
- **Consecuencias:** el CLI (`chat`) y la API (`/v1/chat`, sesiones) enrutan por turno según el provider elegido; `nova doctor` muestra AI mode, privacidad y estado OpenCode. Nuevos tests: `tests/test_opencode.py`, `TestHybridRouting` en `tests/test_router.py`, y extensiones en `tests/test_config.py`/`tests/test_setup.py`. El modo LOCAL de PHASE 7 queda exactamente igual salvo el campo `provider` nuevo en el decision (valor `ollama`). **Validación en vivo (PHASE 14.1/14.2):** el shape real del servidor OpenCode (`GET /config/providers` → `{providers:[{id, models:{...}}], default:{...}}` con `models` como diccionario, y `GET /global/health`/`POST /session`/`POST /session/{id}/message`/`DELETE /session/{id}`) se verificó contra una instancia real 1.18.30; `_opencode_modules()` fue corregido para ese shape (más fallback legacy) y `OpenCodeProvider.chat()` se probó de extremo a extremo contra el servidor real respondiendo `NOVA_OK`. El estado de autenticación se reporta por existencia (`available/not_configured/unknown`), nunca por contenido.
- **Estado:** aceptada.

## ADR-021 — Dependencias reales y UX de install (overhaul de `nova-setup`, PHASE 14.2)

- **Fecha:** 2026-09-09
- **Contexto:** `nova-setup` tenía la detección de dependencias dispersa, mensajes de consola inconsistentes, sin comando de reparación y sin modo máquina; además `_opencode_modules()` no entendía el shape real del servidor OpenCode. Se pide una auditoría de dependencias completa y una experiencia de terminal profesional y legible, sin romper el modo LOCAL, `install.ps1`/`install.sh` ni los 392+ tests.
- **Decisión:**
  1. **Catálogo de dependencias explícito y vinculado al código** (`nova/setup/dependencies.py`): cada dependencia es un `Dependency` (nombre, propósito, binario/módulo, versión mínima, obligatoria, plataforma, feature, método de instalación). Solo dependencias que N.O.V.A. realmente importa o ejecuta (Python 3.11+/venv/pip, `httpx`, `pydantic`, `pydantic-settings`, `PyYAML`, `fastapi`, `uvicorn`, `ollama`; opcionales: `opencode`, voz `numpy/vosk/pyttsx3/sounddevice`, dev `pytest/git`). Nada se inventa.
  2. **Detectar es probar, no "está en PATH"**: se localiza el binario y se solicita su versión real (`--version`); los paquetes se detectan por módulo importable y su versión por metadata. Comparador de versiones propio que admite `3.11`, `2.4.1`, `v1.2.3` (`version_tuple` de `remote_catalog` exigía `X.Y.Z`).
  3. **Instalación delegada y segura**: Python vía `pip` del intérprete activo; Ollama vía `winget` o el instalador oficial solo con confirmación del usuario (interactivo) o automáticamente en `auto`/`repair`; **OpenCode nunca se auto-instala** (ADR-020) y las credenciales no se tocan. Windows es el objetivo primario; Linux/macOS degradan a instrucciones y no se rompen.
  4. **Capa de presentación única** (`nova/setup/terminal.py`) sin dependencias nuevas: colores ANSI (respetando `NO_COLOR`/`TTY`), símbolos Unicode con fallback ASCII, secciones, tabla, spinner y confirmaciones; modo `--json` con una línea JSON por evento (sin decoración parse-hostile) y `--non-interactive`.
  5. **Flujo por fases** en el asistente (System → Dependency → Install → Verify → Hardware → Ollama → OpenCode → Models → Config → Autostart/Voice → Ready) manteniendo la firma `run_assistant()` y los seams (`_ask`, `_say`, `_pull_models`) que los tests usan.
  6. **`nova-setup repair`** (`nova/setup/repair.py`): re-audita, instala faltantes seguros, valida venv sin recrearlo, reinicia Ollama, re-provisiona defaults sin pisar valores del usuario y verifica modelos. Nunca borra recursos ni credenciales.
  7. **`nova doctor` como auditoría** REQUIRED/OPTIONAL/WARNINGS/READY incluyendo auth de OpenCode por existencia (sin leer `auth.json`).
- **Consecuencias:** `install.ps1`/`install.sh` siguen funcionando igual (el `auto` del asistente ahora también espera al final y no cambia los defaults de seguridad). Los tests nuevos (`test_terminal.py`, `test_dependencies.py`, `test_setup_phase14_2.py`) incluyen un E2E de OpenCode real que se salta si el servidor no responde. Se alinea la versión a `0.15.0`. Limitación real documentada: la instalación automática de Ollama y la descarga de modelos requieren red; en ausencia de red se ofrece la instalación manual y `doctor`/`repair` lo señalan.
- **Estado:** aceptada.
